...
<body>
    <div class="container">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-5">
						<h2>Controle <b>de Anúncios</b></h2>
					</div>
					<div class="col-sm-7">
        ...
        $max = 15;
        $str = " $anuncio->categoria ";
        $cat=  substr_replace($str, (strlen($str) > $max ? '...' : ''), $max);

        $max = 15;
        $str = " $anuncio->created_at ";
        $created_at=  substr_replace($str, (strlen($str) > $max ? '...' : ''), $max);
    ...
    ?>
            ...
            <td>
							...
							<a  href="/admin/acessaranuncios/editar/{{ $anuncio->id }}"
                            ...
                            <a href="/admin/acessaranuncios/apagar/{{ $anuncio->id }}"
                               onclick="return confirm('Deseja realmente excluir o anuncio: {{$anuncio->titulo}}?');"
			...
			<div class="clearfix">
                <div class="hint-text">Exibindo <b>{{ $anuncios->count() }}
                                                </b> de <b>{{$anuncios->total()}}</b> anuncios</div>
                        {{ $anuncios->links() }}
            </div>
        </div>
    </div>
